...
public class MissionFour : Mission
{
    ...
    public void changeMainObjective(GameObject current)
    {
        ...
        {
            GameObject.FindObjectOfType<MissionManager>().updateObjectiveTest(new Objective() { title = "Finished Final" });
        }
    }
    ...
    {
        cutscenesParent.transform.GetChild(3).gameObject.SetActive(true);
        this.activeCutScene = cutscenesParent.transform.GetChild(3).gameObject;
        GameObject.FindGameObjectWithTag("Player").gameObject.GetComponent<PlayerMovement>().allowedToMove = false;
        cutscenesParent.transform.GetChild(3).gameObject.GetComponent<PlayableDirector>().stopped += OnPlayableDirectorStopped;
        GameObject.FindGameObjectWithTag("Player").GetComponents<DialogueSystemTrigger>()[3].enabled = true;
    ...
    void OpenNote()
    {
        ...
        ES3.Save<int>("notesCollectedLengthMission_4", inventoryManager.GetComponent<InventoryManager>().notes.Count);
    ...
    void OpenNewspaper()
    {
        ...
        ES3.Save<int>("newsPaperCollectedLengthMission_4", inventoryManager.GetComponent<InventoryManager>().newspapers.Count);
